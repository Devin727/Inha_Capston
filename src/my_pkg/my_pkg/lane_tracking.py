...
# ===== 노란선 =====
YELLOW_LOW         = np.array([15,  30,  80])   # 채도/명도 더 낮춰서 바랜 노란색도 감지
YELLOW_HIGH        = np.array([40, 255, 255])
YELLOW_MIN_AREA    = 5000
YELLOW_MIN_ASPECT  = 5.0
YELLOW_GONE_FRAMES = 5
YELLOW_COOLDOWN    = 8.0  # 노란선 감지 후 쿨다운 (초)

# ===== 교차로 정지 =====
STOP_WATCH_DIST   = 0.3   # 전방 감지 거리 (m)
STOP_CLEAR_SEC    = 3.0   # 물체 사라진 후 대기 시간 (초)
STOP_FRONT_START  = 120   # -3도 idx
STOP_FRONT_END    = 124   # +3도 idx
...
class MissionController(Node):
    ...
    # ══════════════════════════════════════════════════════
    # YELLOW_STOP : 노란선 감지 후 5초간 LiDAR 감시
    # 전방 물체 있으면 정지, 없으면 주행, 5초 후 복귀
    # ══════════════════════════════════════════════════════
    def handle_yellow_stop(self, frame):
        ...
        # 5초 지나면 복귀
        if elapsed >= 5.0:
            self.get_logger().info('[STOP] 5초 종료 → DEFAULT_DRIVE')
            self.transition_to(Mission.DEFAULT_DRIVE)
            return
        ...
        if self.latest_scan is not None:
            ...
            for i in range(STOP_FRONT_START, min(STOP_FRONT_END+1, len(ranges))):
                r = ranges[i]
                if 0.05 < r < STOP_WATCH_DIST:
                    detected = True
                    break

        if detected:
            self.publish_stop()
            self.get_logger().info(f'[STOP] 물체 감지 → 정지 ({elapsed:.1f}/5.0s)', throttle_duration_sec=0.3)
        else:
            self._do_lane_follow(frame)
            self.get_logger().info(f'[STOP] 물체 없음 → 주행 ({elapsed:.1f}/5.0s)', throttle_duration_sec=0.3)
